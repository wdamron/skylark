...
func (enc *Encoder) nextRef() ref {
	return ref(int(enc.values.len) + len(enc.strings) + len(enc.dicts) + len(enc.lists) + len(enc.tuples) + len(enc.funcs))
}
...
func (dec *Decoder) DecodeRef() (byte, ref, error) {
	...
	if tag != T_Ref {
		return 0, ref(0), ErrBadRef
	}
	switch subtag {
	case T_String, T_List, T_Dict, T_Set, T_Tuple, T_Function, T_Funcode:
	default:
		return 0, ref(0), fmt.Errorf("Codec: unsupported reference type; subtag=%v", subtag)
	}
	dec.data = dec.data[2:]
...
func (enc *Encoder) EncodeState(thread *Thread) {
	var frames []*Frame
	didPrologue := false
	for frame := thread.frame; frame != nil; frame = frame.Parent() {
		...
		if fn, ok := c.(*Function); ok {
			enc.EncodeToplevel(fn.funcode.Prog)
			enc.EncodeFnShared(fn)
			didPrologue = true
		}
	}
	...
	enc.WriteUvarint(uint64(len(frames)))
...
func (dec *Decoder) DecodeFrame() (*Frame, error) {
	...
	dec.data = dec.data[1:]
	if tag != T_Frame {
		return nil, fmt.Errorf("Codec: invalid frame tag while decoding: tag=%v", tag)
	}
	frame := &Frame{}
	var x uint64
	var err error
...
func (dec *Decoder) DecodeIdent() (compile.Ident, error) {
	name, err := dec.DecodeString()
	if err != nil {
		return compile.Ident{Name: string(name)}, err
	}
...
func (dec *Decoder) DecodeValue() (Value, error) {
	...
	default:
		...
		return nil, fmt.Errorf("Codec: invalid or unsupported tag while decoding; tag=%v", tag)
	}
}
...
func (dec *Decoder) DecodeStringIterable() (stringIterable, error) {
	...
	if dec.data[0] != T_StringIterable {
		return it, ErrBadTag
	}
	var err error
...
func (dec *Decoder) DecodeIterator() (Iterator, error) {
	...
	default:
		...
		return nil, fmt.Errorf("Codec: invalid iterator type while decoding; tag=%v", tag)
	}
}
